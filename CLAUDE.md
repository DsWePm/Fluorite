...
- 基准：`tools/bench-world.sh [name]` 还原世界 → `./gradlew :fabric:runClient -PbenchWidth=1920 -PbenchHeight=1080 -PbenchWorld=<name>`。帧统计走配置键 `frame-stats.enabled`（`run/config/fluorite.toml`，改前先关游戏），输出在 `run/rt-frame-stats/frame.csv`。**不要在 gradle 命令行加 `-Dfluorite.rt.frameStats=true`**：`-D` 设的是 Gradle 守护进程的系统属性，而构建里没有任何 `-D` 转发到客户端 JVM（`fabric/build.gradle` 只传 `--enable-native-access` 与 `-Xss16m`），所以那个写法静默无效。同名系统属性本身有效，但必须作为客户端 JVM 的 vmArg。
- 诊断：视频设置 → Fluorite Settings → 诊断 → debug view 0–21，**编号连续**（2026-09-09 起：新视图取下一个号，退役视图的编号由后续视图回收，不再保留空洞——旧约定「退役编号不复用」已废止，历史文档里的编号按当时记录理解）。1–7=guide buffer；8–11=体积/水；12=空气透视 froxel、13=体积天空开阔度、14/15=相机前缀、16=云链路、17/18=水仿真、19=远场天空开阔度；20/21=降雨材质与水洼，由 pass A 拥有；各视图定义见 DEVELOPMENT.md §5.3。隔离开关 `water.scatter-source`、`volumetrics.segment-source` 等。
