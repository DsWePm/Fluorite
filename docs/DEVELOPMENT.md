...
| 卷云天顶环 | D160A 已修正天空逃逸云段被通用 `RAY_FAR=10000` 截断的问题，并裁剪末步、在 32000–40000 平滑收敛；契约测试与 shader 编译通过，仍需游戏内确认无硬环且成本可接受 |
...
#### D160–D162 云远距与形态：等待游戏内验收

用户 A/B 已否证旧“球壳/平层边界主导环形”的假设：厚度 8→60 不移动环，风速也不影响环；高度 1800 时环非常明显而低高度减弱。确认根因是天空逃逸后的云 march 仍被地形用的 `RAY_FAR=10000` 截断，固定距离与云高度共同形成锁在世界天顶的角度边界。D160A 为云单独提供 40000 格预算、在 32000–40000 平滑衰减，并把最后一步裁到剩余介质段；D161A/D162A 在不改变云型剖面和物理光照的前提下加入多尺度云包、差分平流与高云形态滑条。完整诊断、失败路线、版权边界和实现记录见[开发日志](devlog/M11-clouds.md#d160d162远距环与云形态重做待游戏内验收)。

验收只看以下边界：高度 512/1091/1800 均不得出现硬角度环；地平线只允许平滑远距收敛；层云保持大片、积云出现小云包、积雨云仍保留塔状轮廓；固定视角观察 2–5 分钟时大形随全局风平移、细节连续演化且不闪烁；高云形态 0/0.35/1 应连续呈现纤维状/混合/圆润卷积云，不得发生亮度或覆盖率跳变。还需用固定场景比较改动前后的云 GPU 时间，未实测前不宣称性能可接受。
