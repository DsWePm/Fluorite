...
单独列出，因为它是当前 Issue #41 的根，而且从任何单个文件里都看不出来：
...
#### Issue #41：矿洞远处雾被无遮挡天空照亮

矿洞里近处雾正确变暗，远处雾却被当作暴露在天空下照亮。

**报告人判断的症状与不对称都成立，但机制不是「某一套估计器没写天空遮挡」**——三条环境路径都乘了可见性网格的 sky 项（marched 走 `skySource * visSample.sky`，闭式与水下/封闭介质走 `sampleVolumeVisibility(gateP).y`）。差别在于**采样点是否落在网格内**：`sampleVolumeLightVisibility` 的默认返回值就是「完全开阔」，出界直接返回它，且在距离任一面 `VIS_EDGE_FADE`（范围的 6%）以内就开始朝「开阔」渐隐。网格是 64×32×64 格、默认格 1 格，即相机周围**水平约 ±32 格、垂直约 ±16 格**。水下之所以看着正确，只是因为它在水面上方半格取样，那个点通常牢牢在网格内。

**「两套雾叠加渲染」一节未证实**：两者确实相加，但按设计覆盖**不相交**的 segment（froxel 管相机前缀、闭式管每次弹射），`volume.slang` 明确把两者的一致性当作承重条件。判定为第二个成因之前，必须先用隔离开关 `volumetrics.segment-source`（flags 位 16–17）与 debug view 8–10 分离。**两处不得同时改**，否则无法归因。

**已修的两半**：D178 把出界策略从「重置为完全开阔」改为**非对称 clamp**（横向与向下 clamp，向上不 clamp，因为天空开阔度沿 +Y 单调非减，clamp 会把下界当测量值）；D179 把网格从雾开关下**解耦**——`visibilityGridOrigin` 与烘焙 dispatch 不再读 `Volumetrics.ENABLED`，改由网格自己的 `VISIBILITY_CELL_SIZE` 管。关雾不再让洞穴水发亮。代价是雾关时仍付 131k 光线/帧；要省这笔用 `volumetrics.visibility-cell-size = 0`。详见 [D178/D179](devlog/M13-fog-weather.md)。

**仍未结的一半**（远场）**已裁决（2026-09-02，M28 S1）**：可见性网格升级为**方向性天空场**——每格跨帧累积带真阴影线的天空方向样本，结构性替换「标量开阔度 × 单一天顶方向」。原候选方向（扩大网格、改变出界回退值、网格边界渐隐天空源）与「提取原版天空光照」（M27，已尝试并于合并前放弃，见 [M27 日志](devlog/M27-mc-skylight-field.md)）一并出局。复验必须站在**四周封闭的深洞里**——洞口站位分不开「正确的露天 veil」和「泄漏的洞内光」，不算证据。见 §8.13。
...
**已知残留**（不得当作未发现的 bug 重新排查）：矿洞里，位于网格射线跨度之外的雾段仍被无遮挡天空照亮——`makeNoisyAmbientLayout` 按网格进出点把 march 切成三段，第 1/3 段**根本不查网格**、直接写死开阔，夹取够不着。以及体积雾整体关闭时网格不烘焙，水介质因此失去天空遮挡。
...
| 切片 | 内容 | 备注 |
| --- | --- | --- |
| S1 | 可见性网格 → 方向性天空场 | 不依赖骨架，可先行独立验收；#41 复验须站封闭深洞 |
...
| S6 | 雾照明接统一链路 + 方向场 | 雾观感可调性是验收重点（M25 教训） |
