...
| S1 | 两个 RIS 调用点的写入 + 时间读取 + 合并；光源身份校验；`RtRestirStats` 按深度的接受率 | **代码已落地，游戏内未验证** |
...
**S1 的接受率统计是本里程碑最重要的一次测量**（`diagnostics.restir-stats`，默认关）：深层弹射顶点的时间复用大概率落不了地——上一帧该像素的第 3 次弹射与这一帧基本不是同一表面，验证会大量拒绝。若第 3 层接受率只有个位数，那 530 MB/层就是在为每帧都被拒绝的数据付钱，压缩档（或直接砍深度）的决定不需要争论。

#### S1 的已知偏差与未验证项

- **有偏且知道偏在哪**：存下的 M 个候选是在上一帧的顶点上抽的，把它们当作这一帧的候选计数就是朴素 ReSTIR 合并。两个顶点对光源看法不同的地方偏暗。**S2 的 MIS 权重是消除它的东西**，不是别的补丁。
- **游戏内一次都没跑过。** 命中率是多少、画面有没有可见改善、开档后的帧时间，全部未知。铁律 7 的测量在此之前不成立。
- 雨天湿面的着色法线带涟漪扰动，逐帧摆动会消耗 26° 的法线窗口。理论上无害（法线变了光照也变了），但未观测。

#### 尚未裁决

- `REUSE_M_CAP = 20` 沿用文献起点，未定价（§8.11）。
- 拒绝阈值 0.05 格 / 26° 是首版取值，未按场景校准。
