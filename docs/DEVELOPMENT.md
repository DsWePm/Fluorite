...
| S1 | 两个 RIS 调用点的写入 + 时间读取 + 合并；光源身份校验；`RtRestirStats` 按深度的接受率 | **已落地并实测一轮**（见下）；d0 重投影已加，效果未测 |
...
d0 的重投影已实现（`projectPrevNdc` 移到 `world_core`，两个 pass 共用一份）：**只有读被重投影，写永远落在自己的像素上**；重投影错了只会漏掉复用不会用错。**重投影后的效果尚未实测。**

#### S1 的已知偏差与未验证项

- **有偏且知道偏在哪**：存下的 M 个候选是在上一帧的顶点上抽的，把它们当作这一帧的候选计数就是朴素 ReSTIR 合并。两个顶点对光源看法不同的地方偏暗。**S2 的 MIS 权重是消除它的东西**，不是别的补丁。
- **容差在远处不够（已算出，未观测）**：即使重投影完全正确，上一帧那个像素打中的点与本帧仍差约半像素视差。533×300 下 10 格处约 0.011 格（0.05 够用），100 格处约 0.115 格（不够），掠射角再除 cos。预期近处救回、远处仍拒。改成平面距离判据是候选方案，**未裁决**。
