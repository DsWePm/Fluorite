...
| M17 发光体 NEE | **+20.9 ms（1.538×）** | 38.812→59.707。一条阴影线不该值 20 ms ⇒ 疑为光源网格依赖加载链（按段跑 vs 表面按着色顶点跑）。**已挂起，判据实验见 `VOLUME_EMITTER_NEE` 注释** |
...
**待办**：⚖ 相位 vs 光源采样的 MIS（仍未定，要噪声带图）· **散射顶点开档后的噪声特征游戏内验收**（默认已改开，但观感未验）· D31 的判据实验 · `bench-water-top` 尚未采（水面之上的雾+水混合场景）。
...
| D31 | 发光体 NEE 处置 | **挂起，保持默认关** | 实测 **+20.9 ms（1.538×）**，远超任何预算。一条阴影线不该值 20 ms ⇒ 疑为光源网格的依赖加载链（本实现**按段**跑，而表面着色**按着色顶点**跑，段数远多于着色顶点；`lighting.slang` 记着那条候选行走值 5.9 ms/18 ms 帧）。**这是假设不是结论**，判据实验已写进 `VOLUME_EMITTER_NEE` 注释：静默阴影线只留网格查询——贵的还在＝机制错（应改为每路径一次，或等 ReSTIR 预采样池），掉下去＝实现可优化。**不在判据之前重写它** |
