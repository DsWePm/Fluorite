...
- **ReSTIR 整合（M24，进行中）**：reservoir 持久存储已落地并验证分配，复用逻辑未接。见 §8.9 与 [M24 日志](devlog/M24-restir.md)。
...
| 切片 | 内容 | 状态 |
| --- | --- | --- |
| 存储 | `restir.slang`：布局、槽位寻址、复用判据；`composite.restir-reuse-depth` 旋钮与缓冲分配 | **已落地**，用户已验证深度 4 分配成功（1080p 约 1 GB） |
| S1 | 写入 + 时间读取 + 合并 + **按弹射深度的命中率统计** | 未接 |
| S2 | 空间复用 + MIS 权重（朴素复用**有偏**） | 未开始 |
| S3 | 动态光接入候选集、移除 `UNWEIGHTED_SPEC_ALPHA_FLOOR` | 未开始 |

存储格式的裁决与理由见 [M24 日志](devlog/M24-restir.md) D180/D181。要点：用户裁定**先做未压缩版再按实测决定是否压缩**（铁律 7），因此格式关在 `packReservoir`/`unpackReservoir`/`reservoirSlot` 三个函数后面，换压缩档不动调用点。**关档（深度 0）与本缓冲存在之前字节级一致。**

**S1 的命中率统计是本里程碑最重要的一次测量**：深层弹射顶点的时间复用大概率落不了地——上一帧该像素的第 3 次弹射与这一帧基本不是同一表面，三重验证会大量拒绝。若第 3 层接受率只有个位数，那 265 MB/层就是在为每帧都被拒绝的数据付钱，压缩档（或直接砍深度）的决定不需要争论。
