...
- **M26 光源预采样池(进行中)**:目标是让**发光体照亮体积**付得起。D207 实测废掉了「只做第一段」——spp=1、1080p 是约 200 万条路径,每条都有 bounce 0,所以第一段就是全屏,地板等于整整多跑一遍光源采样。剩下的唯一候选是 D3 当初就划给 ReSTIR 的**预采样池**:不是让走表变少,而是把走表整个搬出采样点。

  当前 `sampleVolumeEmitter` 是一条 **4 层依赖加载**:格坐标(纯算术)→ `LightGridCell` → `LightGridSpan` → `LightAlias` → `Light`,每一次的地址都要等上一次的结果。这正是 D31 测到的延迟受限形态,也是它当时得出「优化射线没用」的原因。

  **用户已批准的三条(2026-08-23)**:

  1. **池按已填充格定址**。在 `LightGridCell` 里加 `poolRank`(填充格中的序号),池按 `rank * K + k` 排布。采样变成:格坐标 → `LightGridCell` → 池 = **2 层依赖加载**,且比原来浅。**全世界一套代码路径,没有回退分支** —— 相机锚定窗口能压到 1 层但要多一条路径,而这一轮被双路径设计咬过三次(近/远拆分、网格/射线拆分、写/读不一致),不值得。
  2. **池里存解包后的光源**(位置、法线、辐亮度、面积、pdf,约 48 B/条)。只存索引会省 6 倍显存,但采样时还要再跟一次 `Light` 加载,而那次加载的地址依赖池的读出结果——等于又一层依赖,把整个里程碑的目的抵消掉一半。
  3. **先体积,再表面**。第一片只接体积 NEE:它默认关着,所以开关天然就是隔离 A/B,铁律 8 自动成立。第二片再把表面着色接上同一个池——那一片改的是**已发布的主路径**,得有自己的隔离开关和自己一轮验收。

  **尚未决定 / 待实测**:池深 K 的默认值(暂定 8);池是否每帧重抽(倾向是,固定池会让时间累积相关);`WorldPush` 要为池地址和 K 付两条 lane(1168 → 1184),这是一个真实特性该付的 ABI,和 M25 撤除时还回去的那五条性质不同。

  完成判据:`gpu.traceIndirect` 的同会话比值(铁律 7),对照 D31 的 +20.9 ms 基线。
...
**M27 补上的第三半**：远场改读 **Minecraft 自己的天光**（用户 2026-08-27 裁决：「按 mc 的天光给远处雾算亮度」），走 384³ 的 R8 环形场，开关是「远处雾用 MC 天光」。同一轮把 `volumeSkyOpenness` 里 **+Y 出格返回 1.0** 那条也换成读同一个场（用户 2026-08-27 批准，见下条与 #73）——D178 当初留着那个常量的理由是「网格上方没有数据」，M27 之后不成立了。**两条的关档都仍是旧常量**（铁律 8）。

**M27 之后仍未证实的部分**：截至 2026-08-27，所有「洞里远处雾还亮」的现场观察都是**站在洞口往里看**拍的。那个站位下射线前几十格全是露天空气，天空项本来就该强，所以那批证据**分不开「正确的露天 veil」和「泄漏的洞内光」**，不能用来判罪。重做必须站在四周封闭的深洞里。

同时要记着：MC 天光**不是天空可见度，是洪水填充**——开口竖直列里一路 15 不衰减（60 格深天坑底部读满开），横向每格才掉 1（拐进侧洞 5 格还有 67%）。网格算的是余弦加权上半球积分。**同一个点两者可以差一个数量级，且 MC 天光在深洞与侧洞里系统性偏亮**，偏的方向和 #41 原症状相同。「远场用 MC 天光所以正确」不是已证结论。
...
现在不要提前做：独立面光源解析 MIS、预采样池专项优化、过度加固可能被替换的 alias/grid。**以及远场天空可见性的任何结构性方案**——见下。

#### 远场天空可见性（Issue #41 残留，等的是体积 ReSTIR 而不是本节的 ReSTIR）
...
能解决它的是**体积 ReSTIR**：样本空间是「射线上的一个点 × 一个光源」而不只是一个光源，届时每个体积样本可以随机采一个天空方向、射一条真阴影线，由时空复用积出来。那会**一次换掉 §2.8 记的三层近似**（标量辐亮度、单一方向、有限缓存）。它是另一条研究线，不在本节 scope 内。

因此按「是否会被淘汰」分档，已裁决：

- **不会被淘汰，已做**：可见性网格的 miss 策略。横向与向下夹取到边界格；**+Y 不夹**（天空开阔度沿 +Y 单调非减，夹取会把下界当测量值）——但 M27 之后 +Y 也不再返回常量 1.0，改为读 MC 天光场，那不是夹取而是**换一个场测同一个点**。任何缓存都需要合理的 miss 策略，换实现也换不掉这个问题。
- **~~会被淘汰，故推迟~~：提取原版天空光照作为远场来源**——**用户 2026-08-27 直接裁决推翻了这条推迟**（M27 已落地）。理由是现场观感：夹取到边界格会让野外远景雾一起变暗，而洞内亮的问题没解决。铁律 1 的裁决权在用户，这里按裁决执行。
- **仍推迟**：第二级联网格、扩大网格维度、近远两来源混合。**在体积 ReSTIR 裁决前不得开工。**
- **可能反而更有用**：原版天空光照那条。ReSTIR 需要一个廉价近似作为**目标/引导函数**（决定往哪个方向多采样，再用真射线验证），届时它可能从「答案」降级成「引导」而不是被丢弃。现在押注属投机。

**ReSTIR 在这里新增而非消除的负担**：时空复用会让天空项带历史，洞口边界上 reservoir 必须正确作废，否则光穿过时间泄漏。这与本节已为动态光记下的纪律同构。边界问题不消失，只是从**空间边界**变成**时域验证**。

**~~已知残留~~（两条都已修，2026-08-27）**：`makeNoisyAmbientLayout` 切出的第 1/3 段曾**根本不查网格**、直接写死开阔——M27 让这两段改读 MC 天光场（开关「远处雾用 MC 天光」）。体积雾整体关闭时网格不烘焙、水介质失去天空遮挡——D179 已把烘焙从雾开关下解耦，改由 `VISIBILITY_CELL_SIZE` 管。

**当前真正的残留**：#41 在洞内是否还亮，**没有有效证据**（见上文「M27 之后仍未证实的部分」——旧观察全部站在洞口，不成立）。#73 的 +Y 常量已改，等游戏内复验。
