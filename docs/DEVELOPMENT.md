...
### 2026-08-08 体积散射太阳项的 4π 能量误差（D61，用户裁定「修物理 + 同一轮把多次散射重推到位」）
...
| # | 证据 | 结果 |
|---|---|---|
| 1 | **本文档自己写的约定**（§8.4「切片②」那段） | 「天空项裸用是因为 `mediumSkyRadiance` 已经是 1/(4π) 球面积分」——那么太阳项的**基量也必须带 1/(4π)**。代码写的是 `E · 4π·p(θ)`，基量上的 1/(4π) 掉了。**文档描述的约定是对的，实现没照着做** |
...
#### 修正后的数字（用户配置：g=0.8、ω=0.9804、σt=0.019/block）
...
**验收判据**：厚云的隐含反照率必须落在 `R∞` 上（可用同一份 CPU 复算脚本对账），且 ω→1 时趋近 1 而不是 1/4。
