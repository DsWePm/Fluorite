...
| 卷云天顶环 | D160 的 40000 格预算与 32000–40000 渐隐经游戏复验完全不改变环，已回退；仅保留与环无关但能量正确的末步裁剪。真实根因仍未知，高云二维透射率贴图路线等待素材与合成方案裁决 |
...
#### 卷云天顶环与低层云形态：重新设计前待裁决

游戏复验确认 D160 的 40000 格云预算和远端渐隐不改变天顶环，因此固定距离角度只是能解释现象的错误假设，不是真实根因；该实验已完整回退，只有 `min(step, tExit-t)` 末步裁剪作为独立能量修复保留。D161 的低层多尺度位移也未达到视觉目标：体积云仍以大块为主，没有明显生长感或足够复杂的云包。完整失败路线和源码审计见[开发日志](devlog/M11-clouds.md#d160d162远距环与云形态重做游戏复验未通过)。

后续分成两个互不捆绑的决策：高层卷云可改为世界锚定的二维光学厚度/透射率贴图，由现有太阳与天空 Radiance 实时着色；低层体积云只借鉴多尺度密度和差分演化的一般思想，重新设计专用密度函数。普通 RGB HDRI 会把拍摄时的太阳、色温和曝光烘死，不能未经拆层直接覆盖动态大气。素材、贴图数据含义、动画方式、阴影/反射所有权和低层新增采样预算均未批准，实施前必须向用户列出物理差距与 GPU 成本。
