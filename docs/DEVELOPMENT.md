...
### 2026-08-08 体积散射太阳项的 4π 能量误差（D61，代码完成、待游戏内验收）
...
**实现与自动验收（2026-08-09）**：五个站点均改为让归一化 phase 直接消费辐照度，不再乘 `4π`。`RtCloudLighting.diffuseSourceScale` 在 CPU 每帧按上式算出 `A/(4π)=R∞(1+K)/π`，写入既有 `cloudLighting.w`；shader 热路径删掉直射的一次 `4π` 乘法、给扩散加一次尺度乘法，云路径净 ALU 数不增加，WorldPush 大小不变。`RtVolumeEnergyContractTest` 在修复前 2/2 按预期失败，修复后验证四类源项源码契约、CPU→push→shader 接线、三组 `(ω,g)` 的厚板反照率，以及 ω→1 时趋近 1 而不是 1/4；定向任务同时通过完整 Slang 编译。剩余出口是游戏内视觉验收，而不是再调物理常数。
