...
- **一份波场，两个消费者**。位移要的是**高度**，而现有 `waterWaveSpectrum` 只算梯度（做法线不需要高度）。必须补一个高度求值，且梯度应是同一高度函数的解析导数，否则几何与法线会缓慢错开。
- **位移在 GPU 上算，不要在 CPU 再实现一遍波场** —— 两份实现必然漂移。用一个 compute pass 写位移后的顶点缓冲，与着色器共用同一个 Slang 模块。
- 总高度 = 波谱高度 + 仿真高度（与 D41「斜率相加」同构）。
