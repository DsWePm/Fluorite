...
| S2b | 空间复用：成对 MIS、邻居读上一帧半区、平面距离判据、统计分报时间/空间 | **已落地，d0 接受率已测（81–89%）**；d1/d2 只有一帧 |
...
这不是调参问题。`world_primary.rgen.slang:323` 的种子 XOR 了 `frameIndex`，**弹射方向每帧独立重采样**，所以上一帧的弹射顶点是几米外的另一个随机点——0.3% 就是两个独立余弦样本偶然重合的概率。放宽 0.05 格只会变成把上一帧的光贴到别的几何上。详见 [M24 日志](devlog/M24-restir.md) D183。

**旋钮上限不降**（用户裁定）：默认仍是 0，文档改为「1 是唯一有用的档」。保留的理由原本是「S2 的空间复用可能救活 d1」——**S2b 落地后这条理由需要修正**（D185）：邻居读的是上一帧半区，而邻居像素上一帧的 d1 顶点同样是独立的半球采样，与本像素的没有统计差别。真正可能改变 d1 的是**判据**而不是「空间」：D183 那 0.3% 测的是 0.05 格绝对位置判据，而两个落在同一面墙上、相距两米的顶点在平面距离判据下可能通过。统计已改为分开报告时间/空间两个接受率，就是为了让这件事被看见而不是被推断。
...
- **画面收益与帧时间代价全部未知，三个切片都是。** 接受率高只说明判据放行：同一面墙上 12 px 外的邻居看到的光源和本像素几乎一样，其 reservoir 也就几乎是同一个，MIS 权重会把它摊薄。**这是 M24 现在最大的缺口**，也是挡在「ReSTIR 值不值」前面的唯一一项。铁律 7 的 A/B 尚未做。
- **平面距离判据是否救活 d1，只有一帧数据**（d1 空间 15.6% 对时间 0.3%，d2 9.0% 对 0.0%，每档约 9000 次尝试）。方向与 D185 的事前推论一致，但一帧是一个机位；需要一次深度 3 + 邻居 >0 的正式运行才作数。
