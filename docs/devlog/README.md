...
| [M24：ReSTIR 整合](M24-restir.md) | reservoir 持久存储与 d0 重投影、成对 MIS 与空间复用、动态发光体接入、亮度/黑体色温、发光体 MIS 取代 1/0 门 | D180–D192 |
| [M26：光源预采样池](M26-light-pool.md) | 按已填充格定址的预采样池、解包光源直存、走表搬出采样点的实测回收 | D207–D208 |
| [M27：MC 天光场（已尝试，合并前放弃）](M27-mc-skylight-field.md) | 384³ 天光场上传、远雾改读、+Y 出界换场；因 M28 方向性天空场替代而放弃 | — |
| [M28：统一 ReSTIR 骨架与雾照明重构（进行中）](M28-restir-backbone.md) | S1 方向性天空场与出格钳制、+Y 面钳制与水路例外、水体段起点采样近似（D209–D210） | D209– |
