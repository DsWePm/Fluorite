...
public final class VulkanDiagnostics {
    ...
    private static volatile VkQueue lastFluoriteQueue;
    private static volatile String lastFluoriteQueueLabel;
    ...
    public static synchronized void logInstanceLayers(VkInstanceCreateInfo createInfo) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (result != VK10.VK_SUCCESS) {
                FluoriteMod.LOGGER.warn("vkEnumerateInstanceLayerProperties(count) failed: {}", result);
            } else {
                ...
                if (result != VK10.VK_SUCCESS && result != VK10.VK_INCOMPLETE) {
                    FluoriteMod.LOGGER.warn("vkEnumerateInstanceLayerProperties(data) failed: {}", result);
                } else {
                    int layerCount = Math.min(count.get(0), layers.capacity());
                    FluoriteMod.LOGGER.info("Vulkan loader-visible instance layers ({}):", layerCount);
            ...
            FluoriteMod.LOGGER.warn("Failed to enumerate Vulkan instance layers", t);
        }
        ...
        FluoriteMod.LOGGER.info("Vulkan layer/overlay environment: {}",
    ...
    public static void addDeviceFaultExtension(Collection<String> extensions, VulkanPhysicalDevice physicalDevice) {
        ...
        deviceFaultVendorBinaryRequested = faultSupport.vendorBinary()
                && FluoriteConfig.Rt.Diagnostics.HEAVY_CRASH_DIAGNOSTICS.value();
        if (deviceFaultRequested) {
            ...
            FluoriteMod.LOGGER.info("Vulkan device-fault diagnostics requested ({}, vendorBinary={})",
                    VK_EXT_DEVICE_FAULT_EXTENSION_NAME, deviceFaultVendorBinaryRequested);
        } else {
            FluoriteMod.LOGGER.warn("Vulkan device-fault diagnostics unavailable on [{}]",
                    physicalDevice.deviceName());
        }
        nvDiagnosticsRequested = FluoriteConfig.Rt.Diagnostics.HEAVY_CRASH_DIAGNOSTICS.value()
        ...
        if (nvDiagnosticsRequested) {
            ...
            FluoriteMod.LOGGER.info("NVIDIA device diagnostics config requested");
        }
    }
    ...
    public static void logEnabledExtensions(Collection<String> extensions) {
        ...
        FluoriteMod.LOGGER.info("Vulkan device extensions requested ({}): {}", sorted.size(), sorted);
    }
    ...
    public static void probe(VkDevice device) {
        ...
        if (deviceFaultRequested) {
            FluoriteMod.LOGGER.info("Vulkan device-fault diagnostics {}",
                    deviceFaultEnabled ? "enabled" : "FAILED: entry point missing");
        }
        if (nvDiagnosticsRequested) {
            FluoriteMod.LOGGER.info("NVIDIA diagnostics config enabled (shader debug, resource tracking, automatic checkpoints, shader errors)");
        }
    }
    ...
    public static void noteQueueSubmission(VkQueue queue, String label) {
        lastFluoriteQueue = queue;
        lastFluoriteQueueLabel = label;
    }
    ...
    public static void reportDeviceLost(VulkanDevice device, String operation) {
        ...
        VkQueue queue = lastFluoriteQueue;
        if (queue != null) {
            logNvQueueCheckpoints(queue, lastFluoriteQueueLabel);
        }
        try {
            String checkpoints = VulkanUtils.formatCheckpoints(
                    device.checkpointExtension().retrieveCheckpoints(true));
            FluoriteMod.LOGGER.error("Vulkan queue checkpoints:\n{}",
                    checkpoints.isBlank() ? "<none>" : checkpoints);
        } catch (Throwable t) {
            FluoriteMod.LOGGER.error("Failed to retrieve Vulkan queue checkpoints", t);
        }
    ...
    public static void reportDeviceLost(VkDevice device, String operation) {
        ...
        FluoriteMod.LOGGER.error("Vulkan device lost while {}", operation);
        logRuntimeSnapshot();
        if (!deviceFaultEnabled || device == null || device.getCapabilities().vkGetDeviceFaultInfoEXT == 0L) {
            FluoriteMod.LOGGER.error("VK_EXT_device_fault is unavailable; no driver fault details can be queried");
            return;
        }
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (result != VK10.VK_SUCCESS) {
                FluoriteMod.LOGGER.error("vkGetDeviceFaultInfoEXT(counts) failed: {}", result);
                return;
            }
            ...
            if (addressCount != counts.addressInfoCount() || vendorCount != counts.vendorInfoCount()) {
                FluoriteMod.LOGGER.warn("Capping Vulkan fault records to {} (driver reported address={}, vendor={})",
            ...
            if (counts.vendorBinarySize() > MAX_VENDOR_BINARY_BYTES) {
                FluoriteMod.LOGGER.warn("Skipping oversized Vulkan vendor fault binary: {}", formatBytes(counts.vendorBinarySize()));
            }
            ...
            if (result != VK10.VK_SUCCESS && result != VK10.VK_INCOMPLETE) {
                FluoriteMod.LOGGER.error("vkGetDeviceFaultInfoEXT(info) failed: {}", result);
                return;
            }
            FluoriteMod.LOGGER.error("Vulkan device fault: description='{}', addresses={}, vendorRecords={}, vendorBinaryBytes={}",
            ...
            if (addresses != null) {
                for (int i = 0; i < Math.min(addressCount, counts.addressInfoCount()); i++) {
                    ...
                    FluoriteMod.LOGGER.error("Vulkan fault address[{}]: type={}, address=0x{}, precision=0x{}, resource={}",
            ...
            if (vendors != null) {
                for (int i = 0; i < Math.min(vendorCount, counts.vendorInfoCount()); i++) {
                    ...
                    FluoriteMod.LOGGER.error("Vulkan vendor fault[{}]: description='{}', code=0x{}, data=0x{}",
            ...
            FluoriteMod.LOGGER.error("Failed to query VK_EXT_device_fault after device loss", t);
        } finally {
    ...
    private static void logRuntimeSnapshot() {
        ...
        FluoriteMod.LOGGER.error("Vulkan in-flight state: {}", IN_FLIGHT);
        FluoriteMod.LOGGER.error("Vulkan recent breadcrumbs (oldest to newest, {}):", breadcrumbs.size());
        for (String breadcrumb : breadcrumbs) {
            FluoriteMod.LOGGER.error("  {}", breadcrumb);
        }
        long totalBytes = BUFFERS.values().stream().mapToLong(BufferRange::size).sum();
        FluoriteMod.LOGGER.error("Fluorite live BDA buffers: count={}, bytes={}", BUFFERS.size(), formatBytes(totalBytes));
        ...
        if (context != null && memoryHeapCount > 0) {
            try (MemoryStack stack = MemoryStack.stackPush()) {
                ...
                FluoriteMod.LOGGER.error("Failed to collect VMA budgets after device loss", t);
            }
        }
    }
    ...
    private static void logNvQueueCheckpoints(VkQueue queue, String label) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (checkpointCount == 0) {
                FluoriteMod.LOGGER.error("NVIDIA checkpoints for {} queue 0x{}: <none>", label,
            ...
            FluoriteMod.LOGGER.error("NVIDIA checkpoints for {} queue 0x{} ({}):", label,
            ...
            for (int i = 0; i < Math.min(checkpointCount, count.get(0)); i++) {
                ...
                FluoriteMod.LOGGER.error("  stage={} (0x{}), marker=0x{}",
                        pipelineStage(checkpoint.stage()), Integer.toUnsignedString(checkpoint.stage(), 16),
                        Long.toUnsignedString(marker, 16));
            }
        } catch (Throwable t) {
            FluoriteMod.LOGGER.error("Failed to retrieve NVIDIA checkpoints for " + label, t);
        }
    }
    ...
    private static void writeVendorBinary(ByteBuffer binary, long reportedSize) {
        ...
        Path path = Path.of("fluorite-device-fault-" + Instant.now().toString().replace(':', '-') + ".bin")
                .toAbsolutePath();
        ...
        try (FileChannel channel = FileChannel.open(path, StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE)) {
            ...
            FluoriteMod.LOGGER.error("Wrote Vulkan vendor fault binary ({} bytes) to {}", size, path);
        } catch (IOException e) {
            FluoriteMod.LOGGER.error("Failed to write Vulkan vendor fault binary to {}", path, e);
        }
    }
    ...
    private static synchronized void logStartup(VulkanPhysicalDevice physicalDevice) {
        ...
        FluoriteMod.LOGGER.info("Vulkan selected queues: graphics={}, compute={}, transfer={}",
    ...
    private static void logMemoryAndQueues(VulkanPhysicalDevice physicalDevice) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            for (int i = 0; i < memory.memoryHeapCount(); i++) {
                ...
                FluoriteMod.LOGGER.info("Vulkan memory heap[{}]: size={}, flags={}", i, formatBytes(heap.size()),
                        memoryHeapFlags(heap.flags()));
            }
            for (int i = 0; i < memory.memoryTypeCount(); i++) {
                var type = memory.memoryTypes(i);
                FluoriteMod.LOGGER.info("Vulkan memory type[{}]: heap={}, flags={}", i, type.heapIndex(),
            ...
            for (int i = 0; i < queues.capacity(); i++) {
                ...
                FluoriteMod.LOGGER.info("Vulkan queue family[{}]: count={}, flags={}, timestampBits={}",
