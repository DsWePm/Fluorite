...
public final class RtDlssRr {
    ...
    public static boolean enabled() {
        return FluoriteConfig.Rt.DlssRr.ENABLED.value();
    }
    ...
    private static int renderPreset() {
        return FluoriteConfig.Rt.DlssRr.PRESET.value();
    }

    public static int quality() {
        return FluoriteConfig.Rt.DlssRr.QUALITY.value();
    }
    ...
    public boolean evaluate(long cmd, RtImage color, RtImage depth, RtImage motion,
        ...
        try {
            ...
            failed = true;
            FluoriteMod.LOGGER.error("DLSS-RR evaluate failed; RT composite continues without it", t);
            return false;
        }
    }
    ...
    public boolean ensureFeature(long cmd, int renderWidth, int renderHeight, int displayWidth, int displayHeight) {
        ...
        try {
            ...
            if (featureRenderWidth != renderWidth || featureRenderHeight != renderHeight
                ...
                FluoriteMod.LOGGER.info("DLSS-RR feature created: {}x{} -> {}x{} (quality {}, preset {})",
                        renderWidth, renderHeight, displayWidth, displayHeight, quality, preset);
            }
            return true;
        } catch (Throwable t) {
            failed = true;
            FluoriteMod.LOGGER.error("DLSS-RR setup failed; RT composite continues without it", t);
            return false;
        }
    }
    ...
    private void ensureInitialized(VulkanDevice device) {
        ...
        if (!loggedAvailable) {
            ...
            FluoriteMod.LOGGER.info("DLSS Ray Reconstruction available: {}", available);
        }
        if (!available) {
     ...
     * teardown ({@code NgxRuntime.shutdown()} in {@code FluoriteClient.shutdownRt}), so FG can keep using NGX.
     */
    public void destroy() {
        if (((GpuDeviceAccessor) RenderSystem.getDevice()).fluorite$getBackend() instanceof VulkanDevice device) {
            releaseFeature(device);
