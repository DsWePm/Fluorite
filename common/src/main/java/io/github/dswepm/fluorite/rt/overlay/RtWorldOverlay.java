    ...
    public void compositeIntoUiOverlay(RenderTarget main, RtGpuExecutor.GraphicsUse graphicsUse) {
        ...
        try {
            ...
            if (!ready.isEmpty()) {
                ...
                if (targetView == 0L) {
                    FluoriteMod.LOGGER.warn("World overlay: UI overlay target has no Vulkan image view; skipping");
                    return;
                }
            ...
            failed = true;
            FluoriteMod.LOGGER.error("World overlay failed; disabling for this session", t);
        } finally {
