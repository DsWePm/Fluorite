 ...
 * and the device comes up exactly as vanilla. {@code fluorite.rt} is read once here, at
...
public final class RtDeviceBringup {
    ...
    /** Whether device creation reserved a queue handle exclusively for Fluorite terrain work. */
    ...
    public static int computeQueueFamilyIndex() {
        if (!computeQueueReserved()) {
            throw new IllegalStateException("Fluorite compute queue was not reserved");
        }
    ...
    public static void reserveComputeQueue(VkDeviceCreateInfo deviceCreateInfo,
        ...
        if (requestedQueues == null) {
            FluoriteMod.LOGGER.warn("Fluorite RT disabled: Vulkan device creation has no queue requests");
            return;
        }
        ...
        if (selectedFamily < 0) {
            FluoriteMod.LOGGER.warn(
                    "Fluorite RT disabled: no spare compute-capable Vulkan queue is available");
            return;
        }
        ...
        FluoriteMod.LOGGER.info(
                "Reserved Fluorite compute queue family={} index={} (family queues={}, flags=0x{})",
    ...
    private static boolean ommRequested() {
        return FluoriteConfig.Rt.Omm.ENABLED.value();
    }
     ...
     * the OMM engine arrived together in Ada — so {@code fluorite.rt.omm.force} overrides it for a device
     * known to be fine.
     */
    /**
    ...
    private static boolean reflexRequested() {
        return FluoriteConfig.Rt.Reflex.ENABLED.value();
    }

    /** Query every feature boolean Fluorite might enable in one complete Features2 chain. */
    private static FeatureSupport queryFeatureSupport(VulkanPhysicalDevice physicalDevice) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            boolean ommForced = FluoriteConfig.Rt.Omm.FORCE.value();
            ...
            if (ommRequested() && hasOmmExt && !ommAccelerated && !ommForced && !loggedOmmSoftware) {
                ...
                FluoriteMod.LOGGER.info("Opacity micromaps: extension is present but this device reports SER "
                        ...
                        + "wait. Override with -Dfluorite.rt.omm.force=true.");
            }
            ...
            if (hasSerExt && SER_EXT_FEATURE.get(available) && !reorders && !loggedSerInert) {
                loggedSerInert = true;
                FluoriteMod.LOGGER.info("Shader execution reordering: supported but this device reports hint "
    ...
    public static void addFeatures(Args args, VulkanPhysicalDevice physicalDevice) {
        ...
        if (missingExtension != null) {
            if (!loggedUnavailable) {
                ...
                FluoriteMod.LOGGER.warn("Ray tracing unavailable: device [{}] lacks extension {}",
        ...
        if (!support.supportsRt()) {
            if (!loggedUnavailable) {
                loggedUnavailable = true;
                FluoriteMod.LOGGER.warn("Ray tracing unavailable: device [{}] lacks features {}",
    ...
    public static void probe(VkDevice device) {
        if (!rtRequested) {
            FluoriteMod.LOGGER.info("Ray tracing not requested; skipping RT probe");
        ...
        try {
            ...
            FluoriteMod.LOGGER.error("RT probe threw; continuing without RT", t);
        }
    }
}
