package io.github.dswepm.fluorite.ngx;
...
public final class NgxRuntime {
    ...
    public synchronized NgxLibrary acquire(VulkanDevice device) {
        ...
        try {
            ...
            failed = true;
            lib = null;
            FluoriteMod.LOGGER.error("NGX init failed; DLSS features disabled", t);
            return null;
        }
    }
    ...
    public synchronized void shutdown() {
        if (lib != null && initialized
            ...
            try {
                ...
                FluoriteMod.LOGGER.warn("NGX shutdown failed", t);
            }
        }
        initialized = false;
        failed = false;
    ...
    private void init(VulkanDevice device) {
        ...
        if (shim == null) {
            throw new IllegalStateException(PLATFORM_NATIVES.shimName()
                    + " not found (bundled natives or -Dfluorite.ngx.path)");
        }
        Path nativesDir = shim.getParent();
        if (nativesDir != null) {
            List<String> missingFeatures = missingFeatureLibraries(nativesDir);
            if (!missingFeatures.isEmpty()) {
                FluoriteMod.LOGGER.warn("NGX feature libraries {} not found next to {}; those features will be unavailable",
        ...
        Path dataPath = Platform.paths().gameDir().resolve("fluorite-ngx");
        try {
            Files.createDirectories(dataPath);
        } catch (Exception e) {
            FluoriteMod.LOGGER.warn("Could not create NGX data path {}", dataPath, e);
        }
        ...
        FluoriteMod.LOGGER.info("NGX initialized (shim {})", shim);
    }

    private static Path locateShim() {
        String override = FluoriteConfig.Ngx.PATH.get();
    ...
    private static Path extractBundledNatives() {
        Path dir = Platform.paths().gameDir().resolve("fluorite-ngx")
        ...
        try {
            ...
            FluoriteMod.LOGGER.warn("Could not extract bundled NGX natives to {}", dir, e);
            return null;
        }
    }
    ...
    private static List<String> bundledFeatureLibraryNames() {
        ...
        try (InputStream in = NgxRuntime.class.getResourceAsStream(index)) {
            ...
            FluoriteMod.LOGGER.warn("Could not read the bundled NGX native index {}", index, e);
        }
        return names;
    }
    ...
    private record PlatformNatives(String platformDir, String shimName, List<String> exactFeatureNames,
        ...
        private String resourceDir() {
            return "/fluorite/natives/" + platformDir + "/";
        }
