 ...
 *
 * <p>Sized at {@code -Dfluorite.rt.workerThreads} (default {@code clamp(cores/2, 1, 4)}) to leave
...
public final class RtWorkerPool {
    ...
    private static int resolveThreads() {
        return FluoriteConfig.Rt.WORKER_THREADS.value();
    }
    ...
    private synchronized ThreadPoolExecutor executor() {
        if (exec == null) {
            ...
            exec = e;
            FluoriteMod.LOGGER.info("RT worker pool started with {} thread(s)", threads);
        }
        return exec;
    }
