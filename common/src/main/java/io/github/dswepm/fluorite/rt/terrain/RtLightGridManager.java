...
final class RtLightGridManager {
    ...
    private void publish(RtContext ctx, Uploaded uploaded) {
        ...
        if (FluoriteConfig.Rt.Lights.DUMP.value()) dumpNearbyLights(uploaded.data);

        if (FluoriteConfig.Rt.Lights.STATS.value()) {
            FluoriteMod.LOGGER.info("RT light hierarchy {}: {} lights / {} section slots / {} light grid spans / {} KiB",
    ...
    private static void dumpNearbyLights(RtLightHierarchy.Data data) {
        ...
        double radius = FluoriteConfig.Rt.Lights.DUMP_RADIUS.value();
        ...
        for (int light = 0; light < data.lightCount(); light++) {
            ...
            FluoriteMod.LOGGER.info("RT light[{}] world=({}, {}, {}) area={} Le=({}, {}, {})",
                    light, x + data.rebaseX(), y + data.rebaseY(), z + data.rebaseZ(),
                    area, leR, leG, leB);
            dumped++;
        }
        FluoriteMod.LOGGER.info("RT light dump: {} lights within {} blocks", dumped, (int) radius);
    }
