...
final class RtSectionTable {
    ...
    Generation beginWriteGeneration(RtContext ctx, int minCapacity) {
        int newCapacity = FluoriteConfig.Rt.Terrain.SECTION_TABLE_INITIAL_CAPACITY.value();
    ...
    void ensureEmpty(RtContext ctx) {
        if (buffer == null) {
            int initialCapacity = FluoriteConfig.Rt.Terrain.SECTION_TABLE_INITIAL_CAPACITY.value();
