...
public final class RtHdr {
    ...
    public static void logConfig() {
        FluoriteMod.LOGGER.info(
                ...
                FluoriteConfig.Rt.Hdr.enabled(),
                FluoriteConfig.Rt.Hdr.PAPER_WHITE_NITS.value(), FluoriteConfig.Rt.Hdr.PEAK_NITS.value(),
                FluoriteConfig.Rt.Hdr.enabled() ? "HDR display path active" : "SDR display path",
                FluoriteConfig.Rt.Hdr.headroom());
    }
    ...
    public static void logSurfaceCapabilities(VkPhysicalDevice phys, long surface, int chosenFormat) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (r < 0 || count.get(0) == 0) {
                FluoriteMod.LOGGER.warn("HDR: could not enumerate surface formats (result={}, count={})", r, count.get(0));
                return;
            }
            int n = count.get(0);
            ...
            FluoriteMod.LOGGER.info("HDR: chosen swapchain image format={} ({}); surface advertises {} (format,colorSpace) pair(s):",
            ...
            for (int i = 0; i < n; i++) {
                ...
                anyHdr |= hdr;
                FluoriteMod.LOGGER.info("  [{}] format={} ({}), colorSpace={} ({}){}",
                        i, f.format(), formatName(f.format()), cs, colorSpaceName(cs), hdr ? "  <-- HDR-capable" : "");
            }
            if (anyHdr) {
                FluoriteMod.LOGGER.info("HDR: at least one HDR-capable color space is exposed; PQ/HDR10 presentation is available.");
            } else {
                FluoriteMod.LOGGER.warn("HDR: only SDR color spaces are exposed by this Vulkan surface. Enable OS/display HDR; on Linux, use native Wayland with HDR enabled in the compositor.");
            }
        } catch (Throwable t) {
            FluoriteMod.LOGGER.warn("HDR: surface capability enumeration failed: {}", t.toString());
        }
    }
