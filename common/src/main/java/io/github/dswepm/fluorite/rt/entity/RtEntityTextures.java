...
public final class RtEntityTextures {
    /** Bindless array capacity (slot 0 reserved as a fallback texture). {@code -Dfluorite.rt.maxEntityTextures}. */
    public static int maxTextures() {
        return FluoriteConfig.Rt.EntityTextures.MAX_TEXTURES.value();
    }

    /** Enable pack-compiled entity materials; disabled entities use the canonical neutral header. */
    public static boolean entityPbr() {
        return FluoriteConfig.Rt.EntityTextures.PBR.value();
    }
    ...
    private static final Identifier WHITE_LOCATION = Identifier.fromNamespaceAndPath("fluorite", "rt_white");
    ...
    public int slotForAtlas(Identifier atlasLocation) {
        ...
        try {
            ...
            if (!loggedFailure) {
                loggedFailure = true;
                FluoriteMod.LOGGER.warn("RT atlas texture resolution failed for {}", atlasLocation, t);
            }
        }
    ...
    public int whiteSlot() {
        if (!whiteRegistered) {
            ...
            Minecraft.getInstance().getTextureManager()
                    .register(WHITE_LOCATION, new DynamicTexture(() -> "fluorite RT white", image));
        }
    ...
    private Identifier textureLocation(RenderType renderType) {
        ...
        try {
            ...
            Map<String, ?> textures = ((RenderSetupAccessor) setup).fluorite$textures();
    ...
    public long resolveView(RenderType renderType) {
        ...
        try {
            ...
            if (!loggedFailure) {
                loggedFailure = true;
                FluoriteMod.LOGGER.warn("RT entity texture resolution failed for {}", renderType, t);
            }
        }
