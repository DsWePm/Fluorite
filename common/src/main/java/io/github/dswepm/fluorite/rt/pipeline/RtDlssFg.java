...
import io.github.dswepm.fluorite.ngx.NgxLibrary;
 ...
 * with the present-path refactor. Gated by {@code fluorite.rt.fg} (default off) and hardware/driver support.
 */
public final class RtDlssFg {
    public static final RtDlssFg INSTANCE = new RtDlssFg();

    public static boolean enabled() {
        return FluoriteConfig.Rt.Fg.ENABLED.value();
    }
    ...
    public int effectiveMultiFrameCount() {
        int requested = FluoriteConfig.Rt.Fg.MULTI_FRAME_COUNT.value();
    ...
    public void probeAvailabilityOnce() {
        ...
        lib = l;
        if (!l.hasDlssg()) {
            FluoriteMod.LOGGER.warn("DLSS-FG: loaded ngxshim.dll has no DLSSG ABI — rebuild the shim "
                    + "(cmake --build native/ngx_shim/build --config Release)");
            return;
        }
        available = l.dlssgAvailable();
        multiFrameCountMax = l.dlssgMultiFrameCountMax();
        FluoriteMod.LOGGER.info("DLSS Frame Generation available: {} (multi-frame max {})", available, multiFrameCountMax);
    }
    ...
    public boolean ensureFeature(long cmd, int width, int height, int renderWidth, int renderHeight, int backbufferFormat) {
        ...
        try {
            ...
            if (featureWidth != width || featureHeight != height
                ...
                featureBackbufferFormat = backbufferFormat;
                initialized = true;
                FluoriteMod.LOGGER.info("DLSS-FG feature created: {}x{} (render {}x{}, backbuffer format {})",
                        width, height, renderWidth, renderHeight, backbufferFormat);
            }
            return true;
        } catch (Throwable t) {
            failed = true;
            FluoriteMod.LOGGER.error("DLSS-FG setup failed; frame generation disabled", t);
            return false;
        }
    }
    ...
    public boolean evaluate(long cmd,
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            failed = true;
            FluoriteMod.LOGGER.error("DLSS-FG evaluate failed; frame generation disabled", t);
            return false;
        }
    }
